...
async function getModelTopicDetail(topic) {
...
function getAllTopicRealtimeData() {
    ...
    timer = setInterval(() => {
        ...
        // 将更新后的数据写入 JSON 文件
        fs.writeFile(filePath, cacheJson, {
            encoding: "utf-8",
        }, (error) => {
            if (error) {
                fs.writeFile(filePath, JSON.stringify({ msg: "写入数据失败" }, null, 2), { encoding: "utf-8" }, () => { });
            }
        });
    }, 5000);
}
function getTopicRealtimeData(subscribeTopic) {
...
async function querySqlByPg(sql, params) {
    if (!pgConfig) {
...
function createMcpServer() {
    ...
    server.tool("get-topic-detail", "查询某个topic详情，例如“查询topicA”的详情，入参为：{topic:topicA}", { topic: z.string() }, async (args) => {
    ...
    server.tool("get-topic-history-data-by-pg", `输入你想要查询topic历史数据的需求，例如“查询topicA的最近10条数据”，并将输入作为prompt参数传入，例如{prompt:"查询topicA的最近10条数据"}`, {
    ...
    server.tool("get-topic-query-sql", `输入你想要查询topic的SQL语句需求，例如“生成查询topicA的最近10条数据的sql”，并将输入作为prompt参数传入，例如{prompt:"生成查询topicA的最近10条数据的sql"}`, {
