...
| 单只基金获取 | getFundByCode + 缓存 | 避免全量数据请求 |
| 并发净值刷新 | Promise.allSettled | 串行改并发，提升响应速度 |
| 过期缓存降级 | 错误时返回过期数据 | 提升可用性 |

### 缓存 TTL 策略

| 数据类型 | 交易时间 | 非交易时间 | 说明 |
|---------|---------|-----------|------|
| 基金列表 | 30 分钟 | 24 小时 | 变化不频繁 |
| 基金排行 | 10 分钟 | 2 小时 | 每日更新 |
| 估算净值 | 1 分钟 | 30 分钟 | 实时性要求高 |
| 单只基金 | 5 分钟 | 5 分钟 | 用户添加时使用 |
...
| 日期 | 版本 | 变更内容 |
|------|------|----------|
| 2026-03-01 | v1.5.1 | 类型安全修复、性能优化、错误处理增强 |
| 2026-02-28 | v1.5.0 | 新增 LOF/ETF/分红送配功能 |
